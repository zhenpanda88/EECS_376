...
bool BoxInspector::model_poses_wrt_box(osrf_gear::Shipment &shipment_status) {
    ...
    for (int imodel = 0; imodel < num_models; imodel++) {
        if (imodel != i_box) { //if here, have a model NOT the box
            ...
            {
                find pose of each model wrt to world;    
            }

            find pose of box wrt to world;
            
            Using Math: Box(from b to w)^-1 * Model(from m to w);
            //this gets us the model pose wrt to box

            pushback model pose info;
            Pseudo-code ends */    
...
void BoxInspector::compute_shipment_poses_wrt_world(osrf_gear::Shipment shipment_wrt_box,
    ...
    {
        get pose of product;
        transformPoseToEigenAffine3d(pose of product);
        find pose wrt_the_box;
        transform product from box to world;
        transform from Eigen back to pose
        store pose;
    }
    pushback(product poses);  
