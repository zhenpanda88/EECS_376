...
// orphan_models_wrt_world: vector of models that are seen in the box, but DO NOT belong in the box
  ...
  for(number of parts seen)
  {
    if(pose is within tolerance)
      Store pose into "belong in shipment" array;
  }
  
  for(parts not within tolerance)
  {
    re-adjust parts into tolerance;
    store parts into another array;
  }

  create new array and add poses from the two arrays;
  
  for(items in combined array)
  {
    for(items in box)
    {
      if(check to see if part in combined array does not exists in box)
      {
        add part, with correct pose, to the box
      }
      if else(check to see if part in combined array is duplicate of part in box)
      {
        add pose to duplicate item list;
      }
    }
  }
  ...
  push back all needed information;
