...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    //creates new state and gets current state
    nav_msgs::Odometry current_pose;
    current_pose = vec_of_states.front();

    vec_of_states.clear();

    double x_current = current_pose.pose.pose.position.x;
    double y_current = current_pose.pose.pose.position.y;
    double vel_current = sqrt(current_pose.twist.twist.linear.x*current_pose.twist.twist.linear.x);
    double angle_current = current_pose.pose.pose.orientation.z;

    //create new trajectory
    nav_msgs::Odometry des_state = current_pose;
    des_state.twist.twist.angular.z = 0;

    //deceleration trajectory
    while(vel_current>0)
    {
        vel_current -= accel_max_ * dt_;
        des_state.twist.twist.linear.x = vel_current;
        x_current += vel_current * dt_ * cos(angle_current);
        y_current += vel_current * dt_ * sin(angle_current);
        des_state.pose.pose.position.x = x_current;
